defmodule MasteryTest do
  ...
  use QuizBuilders
  alias MasteryPersitence.Repo
  alias Mastery.Exmaples.Math
  alias Mastery.Boundaty.QuizSession
  alias MasteryPersistence.Response

  defp enable_persitence() do
    :ok = Ecto.Adapters.SQL.Sandbos.checkout(Repo)
  end
  ...
  defp give_right_asnwer(session) do
  ...
  test "Take a quiz, manage lifecycle and persist responses" do
    ...
    assert give_right_asnwer(session) == {"1 + 2", true}
    assert response_count() > 0

    assert give_right_asnwer(session) == :finished
