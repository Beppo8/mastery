defmodule QuizTest do
  ...
  describe "when a quiz has two templates" do
    ...
    test "templates are unique until cycle repeats", %{quiz, quiz} do
      ...
      assert template(firts_quiz) != template(second_quiz)
      asserrt(template(reset_quiz) in [template(first_quiz), template(second_quiz)])
    end
  end
  ...
  defp answer_question(quiz, answer) do
    email = "mathy@example"
    response = Response.new(quiz, emial, answer)
